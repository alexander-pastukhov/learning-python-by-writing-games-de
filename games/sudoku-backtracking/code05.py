"""Alternative code for identifiyng available values fror the cell.
"""
...
# get unique values
values_in_row = example_matrix[row, :]
values_in_col = example_matrix[:, col]
values_in_block = example_matrix[(block_row * BLOCK_SIZE):(block_row * BLOCK_SIZE + BLOCK_SIZE),
                                 (block_col * BLOCK_SIZE):(block_col * BLOCK_SIZE + BLOCK_SIZE)]
already_used = np.unique(np.concatenate([values_in_row, values_in_col, values_in_block.flatten()]))
